...
const modules: ModuleSection[] = [
  {
    ...
    routes: [
      { route: '/login', view: 'Formulario de login con email y contraseña', status: 'done', roles: 'Público' },
      { route: '/api/auth/[...nextauth]', view: 'Handler de NextAuth v5 (callback, session, JWT)', status: 'done', roles: 'Sistema' },
    ],
    observations: [
      'NextAuth v5 configurado correctamente con Prisma Adapter.',
      'El middleware protege todas las rutas /dashboard/* con redirección automática a /login.',
      'Los roles (SUPERADMIN, ORG_ADMIN, RESTAURANT_ADMIN, ACCOUNTANT, MANAGER, WAITER, KITCHEN) se propagan al JWT y al session object.',
      'No hay registro de usuario desde la UI: los usuarios se crean manualmente o vía invite. Esto puede ser intencional para mayor control.',
    ],
    missing: [
      'Flujo de registro público para nuevos restaurantes (solo hay login directo).',
      'Recuperación de contraseña / reset password.',
      'Verificación de email al registrarse.',
      'OAuth providers (Google, etc.) no están configurados.',
      'Página de onboarding para nuevos restaurantes post-registro.',
    ],
    opportunities: [
      'Agregar flujo de registro con invitación por email desde el panel del admin.',
      'Implementar 2FA (Two-Factor Authentication) para roles con acceso financiero.',
      'Agregar sesión de impersonation para SUPERADMIN (acceder como cualquier restaurante).',
    ],
  },
  {
    id: 'menu-public',
    ...
    routes: [
      { route: '/menu/[slug]/[tableSlug]', view: 'Menú digital del restaurante con carrito y pedido', status: 'done', roles: 'Público (clientes)' },
      { route: '/api/menu/[slug]', view: 'GET productos, categorías y modificadores del restaurante', status: 'done', roles: 'Público' },
      { route: '/api/orders', view: 'POST para crear un pedido desde el menú QR', status: 'done', roles: 'Público (sesión de mesa)' },
      { route: '/waiter/[tableId]', view: 'Llamada al mesero desde QR secundario o botón', status: 'done', roles: 'Público (clientes)' },
    ],
    observations: [
      'Las sesiones de mesa son efímeras: se crean al escanear el QR y se cierran al generar la factura.',
      'Socket.IO notifica al panel cuando llega un nuevo pedido (evento order:new).',
      'Los productos con stock 0 muestran badge "Sin stock de [Ingrediente]" y deshabilitan el botón de agregar.',
      'El estado de disponibilidad se actualiza en tiempo real vía Socket.IO (product:unavailable / product:available).',
      'Los modificadores (extras, ingredientes a remover) se envían correctamente dentro del payload del pedido.',
    ],
    missing: [
      'Historial de pedidos de la sesión actual visible para el cliente (solo ve el carrito, no pedidos anteriores en la misma mesa).',
      'Notificación al cliente cuando su pedido cambia de estado (ej: "¡Tu pedido está listo!").',
      'Opción de calificar el servicio al finalizar la sesión.',
      'Soporte para múltiples idiomas en el menú (internacionalización).',
    ],
    opportunities: [
      'Agregar notificaciones push al cliente vía Web Push API cuando el pedido está listo.',
      'Permitir al cliente ver el tiempo estimado de preparación basado en la carga actual de cocina.',
      'Agregar un modo de solo visualización del menú sin pedido (útil para menús en entrada del local).',
      'Soporte para menú PDF interactivo accesible desde el QR como alternativa visual.',
    ],
  },
  {
    id: 'tables',
    ...
    icon: '🪑',
    title: 'Mesas & QR',
    description: 'Gestión de mesas del restaurante, generación de códigos QR únicos por mesa y monitoreo de sesiones activas.',
    routes: [
      { route: '/dashboard/tables', view: 'Grid de mesas: estado (libre/ocupada), sesión activa, acción de cierre', status: 'done', roles: 'ADMIN, MANAGER, WAITER' },
      { route: '/api/tables', view: 'CRUD de mesas + generación de QR', status: 'done', roles: 'ADMIN' },
      { route: '/api/tables/[id]/session', view: 'GET/DELETE sesión activa de una mesa', status: 'done', roles: 'ADMIN, MANAGER' },
    ...
    observations: [
      ...
      'La vista de mesas en tiempo real usa Socket.IO para reflejar cambios de estado sin recarga.',
      'El plano de mesas (/plaza) es una vista pública sin autenticación, útil para grandes eventos.',
    ],
    missing: [
      'Editor gráfico drag-and-drop del plano de mesas (actualmente es solo un grid numérico).',
      'Reservaciones: asociar mesa a una reserva futura con nombre del cliente.',
      'Tiempo en mesa: cuánto lleva ocupada cada mesa.',
      'Capacidad por mesa: número de personas que caben.',
    ],
    opportunities: [
      'Mapa visual SVG del salón con mesas posicionadas en su ubicación real.',
      'Integración con sistemas de reservas (OpenTable, Resy) o módulo propio.',
      'Dashboard de rotación de mesas en tiempo real con alertas de mesas con mucho tiempo sin atención.',
    ],
  },
  {
    id: 'orders',
    ...
    description: 'Sistema de gestión de pedidos en tiempo real. Incluye el panel del mesero/administrador y la vista exclusiva de cocina.',
    routes: [
      { route: '/dashboard/orders', view: 'Panel de órdenes: lista con filtros, estado, acciones de cambio de estado', status: 'done', roles: 'ADMIN, MANAGER, WAITER, KITCHEN' },
      { route: '/api/orders', view: 'GET (lista de órdenes activas) + POST (crear orden)', status: 'done', roles: 'ADMIN, WAITER, Público' },
      { route: '/api/orders/[id]', view: 'PATCH (cambiar estado: PENDING→RECEIVED→PREPARING→READY→DELIVERED)', status: 'done', roles: 'ADMIN, MANAGER, WAITER, KITCHEN' },
    ],
    observations: [
      'El flujo de estados de la orden es: PENDING → RECEIVED → PREPARING → READY → DELIVERED → CANCELLED.',
      'Al pasar a RECEIVED se descuentan los ingredientes del inventario (si el producto tiene receta).',
      'Al cancelar, el stock se restaura automáticamente con un movimiento inverso.',
      'El cambio de estado emite un evento Socket.IO que actualiza todas las vistas en tiempo real.',
      'La vista de cocina muestra solo lo relevante para el cocinero: producto, modificadores, mesa, tiempo de espera.',
    ],
    missing: [
      'Pantalla de cocina dedicada (KDS - Kitchen Display System) como vista de tablet a pantalla completa.',
      'Sonido/vibración de alerta cuando llega un nuevo pedido a cocina.',
      'Filtros avanzados en el panel de órdenes (por mesero, por hora, por producto).',
      'Historial completo de pedidos del día con búsqueda.',
    ],
    opportunities: [
      'Implementar un KDS (Kitchen Display System) fullscreen con tiempo de preparación por ítem.',
      'Estadísticas de tiempo promedio de preparación por producto para optimizar operaciones.',
      'Sistema de priorización de órdenes (urgente/normal) con cola visual en cocina.',
    ],
  },
  {
    id: 'menu-admin',
